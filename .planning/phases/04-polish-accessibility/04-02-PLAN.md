...
files_modified:
  ...
  - src/cards.ts
  - src/map.ts
  - src/emptyState.ts (verification only)
autonomous: true
...
must_haves:
  truths:
    ...
    - "Empty state announces 'No locations match your search' when no results found via aria-live region"
    - "Card selection announces 'Selected [location name]' to screen readers"
  artifacts:
    - path: "src/main.ts"
    ...
    - path: "src/emptyState.ts"
      provides: "Empty state aria-live region for search results announcement"
      contains: "aria-live"
  key_links:
    - from: "src/main.ts"
...
<task type="auto">
  ...
  <action>
    ...
    7. EMPTY STATE (src/emptyState.ts):
       ...
       - Text content "No locations match your search" set in show() function ✓ (Phase 3 implementation)
       - VERIFIED: Complete (implemented in Phase 3, Task 2)
