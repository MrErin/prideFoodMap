...
must_haves:
  ...
  artifacts:
    - path: "src/stateManager.ts"
      provides: "Extended state with visibleLayers Set"
      exports: ["toggleLayer"]
...
**Output:**
- Extended StateManager with visibleLayers Set
...
# Existing patterns
@src/stateManager.ts (Observer pattern, will be extended in 03-01)
@src/map.ts (layer control with overlay names: "Community Fridge and Pantry Locations", "Food Donation Sites")
@src/cards.ts (filterCards from 03-01, will be extended)
...
<task type="auto">
  <name>Task 1: Extend StateManager with visibleLayers Set and toggleLayer method</name>
  <files>src/stateManager.ts</files>
  <action>
    Extend FilterState (created in plan 03-01) with layer visibility:
    ...
    DO NOT modify existing SelectionState, FilterState.searchQuery, or other methods.
  </action>
...
<task type="auto">
  ...
  <action>
    Extend filterCards function (from 03-01) to compose search AND layer filters:
    ...
    DO NOT change updateEmptyState call - it should already be imported from 03-01.

    This composes with 03-01's filterCards - just adds the layer parameter.
  </action>
