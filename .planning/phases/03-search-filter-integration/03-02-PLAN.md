...
must_haves:
  truths:
    ...
    - "Screen readers announce 'No locations match your search' when empty state shows (via aria-live region on empty-state element)"
    - "Pressing Escape while searching clears the selection (existing), search remains active"
  artifacts:
    - path: "src/search.ts"
    ...
    - path: "src/emptyState.ts"
      provides: "Empty state UI component with ARIA live region for search result announcements"
  ...
  key_links:
    ...
    - from: "src/main.ts"
      to: "src/stateManager.ts"
      via: "subscribe to searchQuery changes"
    ...
    - from: "src/cards.ts"
      to: "src/emptyState.ts"
      via: "filterCards() calls updateEmptyState() with visible count"
      pattern: "updateEmptyState"
---
...
- Search input UI in HTML and CSS
- Wired up in main.ts with StateManager subscription
  </objective>
...
@src/stateManager.ts (FilterState with searchQuery, Observer pattern)
@src/cards.ts (createCardElement, renderCards, updateCardSelection)
@src/cards.css (card styles, will add .hidden class)
...
<task type="auto">
  <name>Task 1: Create emptyState.ts module with ARIA live region</name>
...
<task type="auto">
  <name>Task 2: Create search.ts module with debounced input handler</name>
  ...
  <action>
    ...
    7. Show/hide reset button based on input value (add/remove 'hidden' class)
  ...
  <done>Debounce function works, setupSearchInput wires up input and reset button with 300ms delay, reset button shows/hides based on input</done>
</task>

<task type="auto">
  <name>Task 3: Add filterCards function and .hidden CSS class to cards.ts</name>
...
<task type="auto">
  <name>Task 4: Add search UI HTML and CSS styles</name>
...
<task type="auto">
  <name>Task 5: Wire up search in main.ts with StateManager subscription</name>
...
6. Empty state has aria-live="polite" set when visible (for screen reader announcement of "No locations match your search")

Manual verification in browser:

- Load page, type "fridge" in search - only fridge cards show
- Type "xyz" - empty state appears with message and aria-live="polite" attribute
- Click Clear button - all cards show, empty state hides and aria-live attribute is removed
...
3. Empty state element sets aria-live="polite" when visible for screen reader announcement of "No locations match your search"
4. Clear/reset button appears when search is active to restore full card list (SRCH-04)
5. Debounce delay is 300ms (prevents excessive DOM updates)
6. All cards use .hidden class (not removed from DOM)
7. Depends on Plan 01 for FilterState foundation
   </success_criteria>

<output>
After completion, create `.planning/phases/03-search-filter-integration/03-02-SUMMARY.md`
</output>
